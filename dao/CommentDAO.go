...
func GetCommentList(pageSize int, pageNum int) ([]Comment, int64, uint) {
	...
	//err = db.Preload("User").Model(&commentList).Limit(pageSize).Offset((pageNum - 1) * pageSize).Order("Created_At DESC").Select("comment.id, article.title,user_id,article_id, user.user_name, comment.content, comment.status,comment.created_at,comment.deleted_at").Joins("LEFT JOIN article ON comment.article_id = article.id").Joins("LEFT JOIN user ON comment.user_id = user.id").Scan(&commentList).Error
	err = db.Order("Created_At DESC").Limit(pageSize).Offset((pageNum - 1) * pageSize).Find(&commentList).Error
	if err != nil {
